...
var linInterp1 = function (x, y, xi, extrapolate = true) {
    ...
    for (let ii = 0; ii < xi.length; ++ii) {
        ...
        if (xi[ii] >= x[n - 2])                                                 // special case: beyond right end
        {
            ...
            var i = iFindByBisection(xi[ii], x, 0, x.length - 1);
        }
...
var akimaInterp1 = function (x, y, xi, save_Mode = false) {
    ...
    for (let i = 0; i < x.length; ++i) {
        var a = Math.abs(u[i + 3] - u[i + 2]);
        var b = Math.abs(u[i + 1] - u[i]);
        if ((a + b) != 0) {
    ...
    for (let i = 0; i < xi.length; ++i) {
        ...
        var k = kFind(xi[i], x, 0, x.length - 1);
...
function set2conf(set_0, windowSize, samples, confidencelevel, progressInfo = ()=>{}) {
    ...
    progressInfo({ status: "Vorbereitung läuft", progress: null });
    ...
    if (set_0.length > maxLength) {
        progressInfo({ status: "Reduziere Modellgröße", progress: null });

        if (set_0.length > 10000) {
            set_0.sort((a, b) => { return a - b; });
        } else {
            dualPivotSort(set_0);
        }

        let prob_0_tmp = linspace(1.0 / (maxLength), 1.0 - 1.0 / maxLength, maxLength);
